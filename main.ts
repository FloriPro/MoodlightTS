...
async function loadTranslation(name: string) {
    ...
    if (availTranslations.includes(name)) {
        ...
        currentTranslation = JSON.parse(await r.text());
        setStorage();
        updateCach();
    } else {
 ...
 *  -Einstellungen
 */
const empty: string = '{"sizeX":"6","Elements":[[["$element.start",["0"]]]],"pictures":[],"ElementPositions":[[0,0]],"FreeElements":[],"animations":[],"projectName":"unset"}';
...
let canvasPreDraw = document.getElementById('PreDraw') as HTMLCanvasElement;
let canvasPreDraw2 = document.getElementById('PreDraw2') as HTMLCanvasElement;
let ctxPreDraw: CanvasRenderingContext2D = <CanvasRenderingContext2D>canvasPreDraw.getContext("2d");
let ctxPreDraw2: CanvasRenderingContext2D = <CanvasRenderingContext2D>canvasPreDraw2.getContext("2d");

let ProjectLoader = document.querySelector("#projectLoader") as HTMLInputElement;
let PictureLoader = document.querySelector("#pictureLoader") as HTMLInputElement;
...
function updateToNewTranslation(Elements: [string, string[]][][]): [string, string[]][][] {
    if (Elements[0][0][0][0] == "$") {
        return Elements
    }
...
function loadProject(jsonLoad: {
    ...
    FreeElements: [string, string[], [number, number]][];
    animations: string[][];
    projectName: string | undefined;
    ledSortType: string | undefined
}, lastUsed?: boolean) {
    //get save
    ...
    try {
        ...
        if (jsonLoad.ledSortType != undefined) { ledSortType = parseInt(jsonLoad.ledSortType) }
        if (jsonLoad.projectName != undefined) { projectName = jsonLoad.projectName; } else { projectName = sprompt("projekt name: ") }
        if (jsonLoad.Scheduler == undefined) { schedulerList = []; } else { schedulerList = jsonLoad.Scheduler; }
        loadSchedules();
...
function createUserEvents() {
    ...
    PictureLoader.addEventListener('change', function (e) {
        var fileList = PictureLoader.files as any;
        console.log(fileList);
 ...
 * 1: line by line
 */
const moodlightSizeType: { [key: string]: number } = { "6x6": 0, "8x8": 1 }
function firmwareToSettingsCheck(firmware: string) {
...
async function sizeCheck(sizeX: number, sizeY: number, force?: boolean) {
    ...
    if (moodLightSizeX != sizeX || moodLightSizeY != sizeY) {
        if (await cconfirm("Möchtest du das Projekt an die größe des MoodLight anpassen?")) {
...
function elementLenght(Element: [string, string[]]) {
    ...
    if (lengthStore[mapElement(Element)] != undefined) {
        l = lengthStore[mapElement(Element)] as number;
    } else {
        setFont(font, ctx);
    ...
    "$menu.add": () => {
        ...
        Question = ["$question.add", {
            ...
            "$question.add.answer.animation": function (seId) {
                goTo("PictureEdit", 0); mouse[0] = false; resetPicEdit(); animationId = animations.length; animations.push(["000000".repeat(32)]); pictureEditType = 1;
            },
        }]
    },
    "$menu.edit": () => {
...
const settings: { [hauptgruppe: string]: { [einstellung: string]: (callType/*false: lookup, true: click*/: boolean) => string /*type*/ } } = {
    "$settings.general": {
        ...
        "$settings.general.promptInput": function (callType) { if (!callType) { return "bool"; } else { return ""; } },
        ...
        "$settings.general.rightclick": function (callType) { if (!callType) { return "bool"; } else { return ""; } },
        "": function (callType) { return "info" },
    ...
    "$settings.mqtt": {
        ...
        "$settings.mqtt.deleteData": function (callType) { if (!callType) { return "button"; } else { myTopic = ""; myUser = ""; myPass = ""; if (client.isConnected()) { client.disconnect() }; return ""; } },
        "$settings.mqtt.viewData": function (callType) { if (!callType) { return "button"; } else { aalert("Topic: " + myTopic + " | User: " + myUser + " | Password: " + myPass); return ""; } },
        ...
        "$settings.mqtt.showNameOnSend": function (callType) { if (!callType) { return "bool"; } else { return ""; } },
        "$settings.mqtt.delay": function (callType) { if (!callType) { return "num"; } else { return ""; } },
    ...
    "$settings.look": {
        ...
        "$settings.look.info.colors:": function (callType) { if (!callType) { return "info"; } else { return ""; } },
        ...
        "$settings.look.ownDesign": function (callType) {
            if (!callType) { return "button"; } else {
                var l = localStorage.getItem("!designs");
                if (l != undefined) { var d: { [name: string]: string } = JSON.parse(l); } else { aalert("$alert.noCustomDesign"); return ""; } //set JSON parsed var d
                goTo("Question", 1);
        ...
        "$settings.look.maxFPS": function (callType) { if (!callType) { return "num"; } return ""; },
        ...
        "$settings.moodlight.setBank": function (callType) {
            if (!callType) {
                ...
                for (var i = 0; i < alphabet.length; i++) {
                    a[alphabet[i]] = function (seId: number) {
                        send("$" + alphabet[seId])
                        send("L00");
                        goTo("Settings", 1);
                    }
    ...
    "$settings.language": {
        ...
        "$settings.language.byGoogle": function (callType) { if (!callType) { return "info"; } else { return ""; } },
    },
    "$settings.about": {
        "$settings.about.1": function (callType) { if (!callType) { return "info"; } else { return ""; } },
        "": function (callType) { if (!callType) { return "info"; } else { return ""; } },
        " ": function (callType) { if (!callType) { return "info"; } else { return ""; } },
        "  ": function (callType) { if (!callType) { return "info"; } else { return ""; } },
...
const allwaysRedo: { [key: string]: [number] } = { "$element.animation": [0] };
...
function drawColerRect(posx: number, posy: number, sizeX: number, sizeY: number, colorStr: string, ctx: any, draw: any) {
    if (colorStr == "#R") {
...
let Elements: [string, string[]][][] = [[["$element.start", ["0"]]]];
...
function setFont(font: string, ctx: CanvasRenderingContext2D) {
...
function updateScreen(onlyDraw?: any) {
                                    ...
                                    if (Elements[ElementLoadPos][ElementList][0] == "$element.end") {
                                        ...
                                        if (!isKeyDown("alt")) {
                                            ...
                                            if (["$element.loop", "$element.infiniteLoop"].includes(Elements[ElementLoadPos][ElementList][0])) {
                                                ...
                                                while (indentation > 0) {
                                                    it++;
                                                    if (["$element.loop", "$element.infiniteLoop"].includes(Elements[ElementLoadPos][it][0])) { indentation++; }
                                                    if ("$element.end" == Elements[ElementLoadPos][it][0]) { indentation--; }
                                                }
                ...
                if (FreeElements[mouseDataLeft][2][1] > ElementPositions[HoldingEnd][1] + (blockheight - 20) && FreeElements[mouseDataLeft][2][1] < ElementPositions[HoldingEnd][1] + blockheight * (Elements[HoldingEnd].length + 1)) {
                    ...
                    while (true) {
                        ...
                        if (Elements[HoldingEnd][i] != undefined) {
                            if (Elements[HoldingEnd][i][0] == "$element.end") { indentation--; }
                            if (["$element.loop", "$element.infiniteLoop"].includes(Elements[HoldingEnd][i][0])) { indentation++; }
                        }
                        i++;
                    }
        ...
        for (let ElementLoadPos = 0; ElementLoadPos < Elements.length; ElementLoadPos++) {
            ...
            for (let ElementList = 0; ElementList < Elements[ElementLoadPos].length; ElementList++) {
                ...
                if (ElementList != 0) {
                    ...
                    if ("$element.end" == Elements[ElementLoadPos][ElementList][0]) { px += 10 }
                ...
                if (["$element.loop", "$element.infiniteLoop"].includes(Elements[ElementLoadPos][ElementList][0])) { indentation++; }
                if ("$element.end" == Elements[ElementLoadPos][ElementList][0]) {
                    x = 0
                    var addit = 20
                ...
                if (settings[hauptgruppe[settingsSelLeft]] != undefined) {
                    ...
                    if (sel != undefined) {
                        if (settings[hauptgruppe[settingsSelLeft]][sel](false) == "button") {
                            ...
                            var answer: string | undefined = pprompt(setSettings[sel] + " verändern zu");
                            if (answer != undefined) {
