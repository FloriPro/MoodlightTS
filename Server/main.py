...
styleData = { "background": "#fcfcfc", "backgroundPoints": "#646464", "blockArgBackground": "#ffffff", "blueBlock": "#0082ff", "blueBlockAccent": "#0056aa", "YellowBlock": "#ffd000", "YellowBlockAccent": "#aa8a00", "PurpleBlock": "#d900ff", "PurpleBlockAccent": "#9000aa", "MoveBlockShaddow": "#b0b0b0", "EditMenu": "#d0f7e9", "EditMenuAccent": "#7bc9ac", "NormalText": "#000000", "MenuButtons": "#000000", "MenuBackground": "#000000", "MenuText": "#ffffff", "settingsBoolTrue": "#00ff00", "settingsBoolFalse": "#ff0000", "settingsSelMouseOver": "#d2d2d2", "settingsSelStandard": "#dcdcdc", "settingsSelSelected": "#c8c8c8", "backgroundBlur": "#000000", "settingsBackground": "#ffffff", "settingsBackgroundHighlight": "#f0f0f0", "questionRedBackgroundBlur": "#960000", "questionBackground": "#aaaaaa", "objectSidebarBlur": "#c0c0c0", "ProjectName": "#4287f5" }
...
def uploadStyle():
    if google_auth.is_logged_in():
        ...
        with shelve.open("data") as shelv:
            if (shelv["styles"]==None): shelv["styles"]={}
            d=shelv["styles"]
            d[json.loads(dat)[1]+"_"+google_auth.get_user_info()["id"]+"_"+google_auth.get_user_info()["given_name"]]=base64.b64encode(dat.encode('UTF-8'))
            shelv["styles"]=d
        return "ok"
            ...
            try:
                ...
                for d in da:
                    ...
                    if d[0]=="Bestimmte Uhrzeit":
                        if not madeMin:
                            if int(d[3].split(":")[0])==time.localtime(time.time()).tm_hour and int(d[3].split(":")[1])==time.localtime(time.time()).tm_min:#str(time.localtime(time.time()).tm_hour)+":"+str(time.localtime(time.time()).tm_min)==d[3]:
                                exec=True
                        ...
                        if not madeMin:
                            ...
                            with shelve.open("otherData") as s:
                                if d[3] not in list(s.keys()):
                                    s[d[3]]=hashlib.sha512(t.encode("UTF-8")).hexdigest()
                                    exec=True
                                else:
